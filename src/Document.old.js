/*
 * When interacting with Firestore Rest API, we do not receive, and cannot
 * send raw objects to represent the data. Instead, Firestore has a specific
 * way for representing documents and different types inside a document.
 *
 * In the comments below I use the term Raw-Data or JSON when referring to
 * that format.
 *
...
class Document {
	...
	constructor(data = {}) {
		...
		if (!isObjectLiteral(data)) throw Error('Document constructor should receive an Object.');
	...
	static isRawDocument(obj) {
		// A Firestore document is must have these three keys.
	...
	static diff(modified, reference = modified.__reference__) {
		...
		for (let key in modified) {
			...
			// Diff the rest.
			if (!dequal(modified[key], reference[key])) {
		...
		if ('__meta__' in modified) diffObj.__meta__ = Object.assign({}, modified.__meta__);

		if ('__reference__' in modified) diffObj.__reference__ = Object.assign({}, modified.__reference__);
	...
	static mask(obj) {
		 ...
		 */
		const masks = Object.keys(obj).filter(value => value !== '__reference__' && value !== '__meta__');
	...
	static parse(fireDocument, targetObj = {}) {
		...
		if (!isObjectLiteral(fireDocument)) throw Error('The argument is not a valid firestore document.');

		// Validate that we got an object for the target.
		if (!isObjectLiteral(targetObj)) throw Error('The target object must be an object.');
	...
	static composeValue(property) {
		...
		if (type === 'arrayValue') {
			value.arrayValue = {};
			value.arrayValue.values = property.map(subProp => this.composeValue(subProp));
			return value;
		}
		...
		for (let key in obj) {
			...
			if ('__meta__' in obj) {
				for (let key in obj.__meta__) {
					...
					document[key] = key.endsWith('Time') ? obj.__meta__[key].toISOString() : obj.__meta__[key];
				}
			}
